...
from scenario_lib.config import merge_configs, parse_config, print_configs
...
class InferModel(object):
    ...
    def predict(self):
        ...
        result = {}
        for video_id, prob in zip(video_ids, probs):
            result[video_id] = [round(float(score), 5) for score in prob]
            # result[video_id] = [float(score) for score in prob]
        return result
...
if __name__ == "__main__":
    args = parse_args()
    args.output = "/data02/changqing/ZyMultiModal_Data/annotations/v3_cls301/AttentionLstmErnie_epoch13_acc76.1_trainval_outputs.json"
    infer(args)
